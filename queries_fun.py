...
# This function returns employee_id and working days of employees who provide service i
...
# Return names & surnames of employees based on service
...
## Return serv_emp id based on employee id and service name
...
def check_user_busy(tgid:int, date:str, time:str):
    df = show_booking()
    datef = df.execution_date.isin([date])
    timef = df.execution_time.isin([time])
    tgidf = df.telegram_id.isin([tgid])
    res = df.loc[datef & timef & tgidf].shape[0]
    return res+1