...
def add_days(path: str, id: int, *args):
    ''' 
    Adds working days for employees by employee id 
    ''' 
...
def check_foreign_match(path: str, val_check: int, r_tab_col: dict) -> bool:
    ''' 
    Checks if the desired value exists in a reference column
    ''' 
...
def add_hours(path: str, wd_id: int, *args):
    ''' 
    Adds working hours for certain employee at specific day (due to wd_id)
    '''
